...
'''
inputpath='~/Documents/pankoo_04/all_test_ready_data_0630.csv'
data=pd.read_csv(inputpath,index_col=False)
...
for i in range(1,len(data_shop)-28):
    ...
    data_shop.loc[i,'package_count_28']=data_shop.loc[i+1:i+28,'PACKAGE_TODAY'].mean()
    data_shop.loc[i,'takeout_count_28']=data_shop.loc[i+1:i+28,'TAKEOUT_TODAY'].mean()
    data_shop.loc[i,'SP_count_28']=data_shop.loc[i+1:i+28,'SP_TODAY'].mean()
...
outputpath='~/Documents/pankoo_04/all_test_ready_data_month_0630.csv'
